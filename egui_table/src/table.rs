...
use egui::{Align, Context, Id, IdMap, Layout, NumExt as _, Rangef, Rect, Response, Ui, UiBuilder, Vec2, Vec2b, vec2};
use vec1::Vec1;
...
impl HeaderRow {
  pub fn new(height: f32) -> Self {
    Self { height, groups: Default::default() }
  }
}
...
impl Table {
  ...
  #[inline]
  pub fn scroll_to_columns(mut self, columns: RangeInclusive<usize>, align: Option<Align>) -> Self {
  ...
  fn get_row_top_offset(&self, ctx: &Context, table_id: Id, table_delegate: &dyn TableDelegate, row_nr: u64) -> f32 {
...
impl TableSplitScrollDelegate<'_> {
  ...
  fn get_row_top_offset(&self, row_nr: u64) -> f32 {
    self.table.get_row_top_offset(&self.egui_ctx, self.id, self.table_delegate, row_nr)
  }
    ...
    for (row_nr, header_row) in self.table.headers.iter().enumerate() {
      ...
      for (group_index, col_range) in groups.into_iter().enumerate() {
        ...
        if 0 < start && self.table.columns[start - 1].resizable && ui.clip_rect().x_range().contains(header_rect.left())
        {
          ...
          update(&mut self.visible_column_lines, start - 1, ColumnResizer { scroll_offset, top: header_rect.top() });
        }
        ...
        if auto_size_this_frame {
          ...
          header_rect.max.x =
            header_rect.min.x + self.table.columns[start..end].iter().map(|column| column.range.min).sum::<f32>();
        }
        ...
        self
          .table_delegate
          .header_cell_ui(&mut cell_ui, &HeaderCellInfo { group_index, col_range, row_nr, table_id: self.id });
        ...
        if start + 1 == end {
          ...
          *width = width.max(cell_ui.min_size().x);
          ...
          if column.resizable && ui.clip_rect().x_range().contains(header_rect.right()) {
            update(&mut self.visible_column_lines, col_nr, ColumnResizer { scroll_offset, top: header_rect.top() });
          }
        }
      }
    }
  ...
  fn region_ui(&mut self, ui: &mut Ui, scroll_offset: Vec2, columns: Range<usize>, do_prefetch: bool) {
    ...
    let mut col_range = if self.table.columns.is_empty() || viewport.left() == viewport.right() {
      ...
      let col_idx_at = |x: f32| -> usize {
        self.col_x.partition_point(|&col_x| col_x < x).saturating_sub(1).at_most(self.table.columns.len() - 1)
      };
    ...
    if do_prefetch {
      ...
      debug_assert!(self.has_prefetched, "SplitScroll delegate methods called in unexpected order");
    }
    ...
    for row_nr in row_range {
      ...
      for col_nr in col_range.clone() {
        ...
        let mut cell_rect =
          Rect::from_x_y_ranges(self.col_x[col_nr]..=self.col_x[col_nr + 1], y_range).translate(-scroll_offset);
        ...
        self.table_delegate.cell_ui(&mut cell_ui, &CellInfo { col_nr, row_nr, table_id: self.id });
    ...
    for col_nr in col_range.clone() {
      let column = &self.table.columns[col_nr];
      if column.resizable {
        update(&mut self.visible_column_lines, col_nr, ColumnResizer { scroll_offset, top: *self.header_row_y.last() });
      }
    }
  }
}
...
impl SplitScrollDelegate for TableSplitScrollDelegate<'_> {
    ...
    if self.table.scroll_to_columns.is_some() || self.table.scroll_to_rows.is_some() {
      ...
      if let Some((column_range, align)) = &self.table.scroll_to_columns {
        ...
        let x_from_column_nr =
          |col_nr: usize| -> f32 { ui.min_rect().left() + (self.col_x[col_nr] - scrollable_col_x_base) };
      ...
      if let Some((row_range, align)) = &self.table.scroll_to_rows {
        let y_from_row_nr = |row_nr: u64| -> f32 { ui.min_rect().top() + self.get_row_top_offset(row_nr) };
  ...
  fn finish(&mut self, ui: &mut Ui) {
    ...
    for (col_nr, ColumnResizer { scroll_offset, top }) in &self.visible_column_lines {
      ...
      let column_width = self.state.col_widths.entry(column_id).or_insert(column.current);
      ...
      let line_rect = egui::Rect::from_x_y_ranges(x..=x, yrange).expand(ui.style().interaction.resize_grab_radius_side);

      let resize_response = ui.interact(line_rect, column_resize_id, egui::Sense::click_and_drag());
